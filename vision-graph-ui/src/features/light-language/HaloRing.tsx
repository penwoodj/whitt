...
const HaloWrap = styled.div<{ $state: HaloRingProps['state']; $isLive: boolean; $level: number }>`
  position: absolute;
  inset: -8px;
...
export function HaloRing({ state, isLive = false, children }: HaloRingProps) {
  const mockAnalyser = {
  ...
  return (
    <HaloWrap $state={state} $isLive={isLive} $level={level} data-testid="halo">
      {children}
    </HaloWrap>
  )
}
