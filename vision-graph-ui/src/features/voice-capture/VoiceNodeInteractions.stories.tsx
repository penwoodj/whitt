...
const meta: Meta<typeof TestComponent> = {
  title: 'Features/Voice Capture/VoiceNode/Interactions',
...
export const VOX06ClickPinsTooltip: Story = {
  name: 'slice02 -- VOX-06 click pins tooltip',
  args: {
  ...
  play: async ({ canvasElement }) => {
    ...
    const pinBtn = canvas.getByTestId('pin-btn')
    ...
    await waitFor(() => {
      expect(canvas.getByTestId('tooltip-pinned')).toHaveTextContent('true')
    })
  },
}

export const VOX11ClickOutKeepsRecording: Story = {
  name: 'slice02 -- VOX-11 click-out keeps recording',
  args: {
    engine: createFakeSttEngine(['test']),
  },
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(startBtn)
    voiceInputRef.current.setTooltipVisible(true)
    voiceInputRef.current.toggleTooltipPin()
    ...
    await waitFor(() => {
      expect(canvas.getByTestId('tooltip-visible')).toHaveTextContent('false')
      expect(canvas.getByTestId('is-rec')).toHaveTextContent('true')
    })
  },
}

export const VOX17PinnedSurvivesUnhover: Story = {
  name: 'slice02 -- VOX-17 pinned survives unhover',
  args: {
    engine: createFakeSttEngine(['test']),
  },
  play: async ({ canvasElement }) => {
    ...
    voiceInputRef.current.setTooltipVisible(false)
